...
export const ZendeskBatchReportView: React.FC<ZendeskBatchReportViewProps> = ({ 
  ...
  const categories = ['All', 'Inquiry', 'Issue', 'Request'];
  
  // Counts should reflect what's actually being displayed (selected and filtered)
  const selectedReports = individualReports.filter(r => selectedTicketIds.has(r.ticketId));
  const categoryCounts = {
    All: selectedReports.length,
    Inquiry: selectedReports.filter(r => r.category === 'Inquiry').length,
    Issue: selectedReports.filter(r => r.category === 'Issue').length,
    Request: selectedReports.filter(r => r.category === 'Request').length,
  };
      ...
      <section className="bg-white rounded-3xl border border-morandi-yellow-100 shadow-xl overflow-hidden">
        <div className="bg-morandi-yellow-600 p-8 text-white">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            {/* Category Filter */}
            <div className="flex p-1 bg-black/10 rounded-xl overflow-x-auto whitespace-nowrap">
              {categories.map(cat => (
                <button
                  key={cat}
                  ...
                  className={cn(
                    ...
                    activeCategory === cat && cat === 'All' && "text-slate-800",
                    activeCategory === cat && cat === 'Inquiry' && "text-blue-600",
                    activeCategory === cat && cat === 'Issue' && "text-rose-600",
                    activeCategory === cat && cat === 'Request' && "text-emerald-600"
                  )}
                >
                  {cat}
                  <span className={cn(
                    ...
                    activeCategory === cat 
                      ? (
                          cat === 'All' ? "bg-slate-100 text-slate-600" :
                          cat === 'Inquiry' ? "bg-blue-50 text-blue-600" :
                          cat === 'Issue' ? "bg-rose-50 text-rose-600" :
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-morandi-yellow-50/50 border-b border-slate-200">
                  ...
                  <th className="p-4 text-sm font-bold text-slate-700 border-r border-slate-200">案件說明</th>
                  <th className="p-4 text-sm font-bold text-slate-700">To do</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredReports.filter(r => selectedTicketIds.has(r.ticketId)).map((report) => (
                  <tr key={report.ticketId} className="hover:bg-slate-50 transition-colors align-top">
                    <td className="p-4 text-center border-r border-slate-100">
                      {report.category ? (
                        <span className={cn(
                          "px-2 py-1 rounded-lg text-[10px] font-bold uppercase",
                          report.category === 'Inquiry' && "bg-blue-50 text-blue-600",
                          report.category === 'Issue' && "bg-rose-50 text-rose-600",
                          report.category === 'Request' && "bg-emerald-50 text-emerald-600",
                        )}>
                          {report.category}
                        </span>
                      ) : (
                        <span className="text-slate-300 text-[10px] font-bold italic opacity-30">NONE</span>
                      )}
                    </td>
                    <td className="p-4 text-center border-r border-slate-100">
                      <a 
