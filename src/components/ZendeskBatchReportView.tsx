...
interface ZendeskBatchReportViewProps {
  ...
  onUpdateSelection: (selectedIds: Set<string>) => void;
...
const categoryColors: Record<string, { bg: string, text: string, border: string, active: string }> = {
  Inquiry: { bg: "bg-blue-50", text: "text-blue-600", border: "border-blue-100", active: "bg-blue-600" },
  Issue: { bg: "bg-red-50", text: "text-red-600", border: "border-red-100", active: "bg-red-600" },
  Request: { bg: "bg-emerald-50", text: "text-emerald-600", border: "border-emerald-100", active: "bg-emerald-600" },
  All: { bg: "bg-slate-50", text: "text-slate-600", border: "border-slate-100", active: "bg-slate-700" }
};

const getCategoryColor = (cat: string | undefined) => {
  if (!cat) return categoryColors.All;
...
export const ZendeskBatchReportView: React.FC<ZendeskBatchReportViewProps> = ({ 
  ...
  const categories = ['All', ...Array.from(new Set(individualReports.map(r => r.category).filter(Boolean))) as string[]];
  ...
  const visibleReportsInTable = filteredReports.filter(r => selectedTicketIds.has(r.ticketId));
        ...
        <div className="bg-morandi-yellow-600 p-8 text-white">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="space-y-4">
              ...
              <h2 className="text-4xl font-black tracking-tight">Chat Duration 超時原因分析報告</h2>
            </div>

            {/* In-Header Category Filter */}
            {categories.length > 1 && (
              <div className="flex flex-wrap gap-2 self-end mb-1">
                {categories.map(cat => {
                  ...
                  return (
                    ...
                    >
                      <span className={cn(!isActive && colors.text)}>
                        {cat}
                      </span>
                      <span className="ml-1 opacity-50">({individualReports.filter(r => cat === 'All' || r.category === cat).length})</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {visibleReportsInTable
              .filter(r => batchSummary.caseIds.includes(r.ticketId))
              .map(r => {
                const colors = getCategoryColor(r.category);
                return (
                  <a 
                    key={r.ticketId} 
                    href={`https://furbo.zendesk.com/agent/tickets/${r.ticketId}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={cn(
                      "px-3 py-1 rounded-full text-xs font-bold transition-all flex items-center gap-1 border shadow-sm",
                      colors.bg,
                      colors.text,
                      colors.border,
                      "hover:opacity-80 active:scale-95 blur-none backdrop-blur-none"
                    )}
                  >
                    #{r.ticketId}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-morandi-yellow-50/50 border-b border-slate-200">
                  <th className="p-4 text-sm font-bold text-slate-700 w-24 text-center border-r border-slate-200">項目</th>
                  <th className="p-4 text-sm font-bold text-slate-700 w-28 text-center border-r border-slate-200">工單號</th>
                  <th className="p-4 text-sm font-bold text-slate-700 w-28 text-center border-r border-slate-200">Chat 時長</th>
                  <th className="p-4 text-sm font-bold text-slate-700 border-r border-slate-200 w-[35%]">案件說明</th>
                  <th className="p-4 text-sm font-bold text-slate-700 w-[35%]">To do</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {visibleReportsInTable.map((report) => {
          ...
          {!isSharedView && (
            <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-slate-200 shadow-sm">
              <input 
                ...
                checked={filteredReports.length > 0 && filteredReports.every(r => selectedTicketIds.has(r.ticketId))}
                onChange={() => {
                  const next = new Set(selectedTicketIds);
                  const allInFilteredSelected = filteredReports.every(r => next.has(r.ticketId));
                  if (allInFilteredSelected) {
                    filteredReports.forEach(r => next.delete(r.ticketId));
                  } else {
                    filteredReports.forEach(r => next.add(r.ticketId));
                  }
                  onUpdateSelection(next);
                }}
              />
              <label htmlFor="select-all-bottom" className="text-[10px] font-bold text-slate-500 cursor-pointer">全選此項目分享</label>
            </div>
          )}
        </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {report.category && (
                      <span className={cn(
                        ...
                        getCategoryColor(report.category).bg,
                        getCategoryColor(report.category).text,
                        getCategoryColor(report.category).border
                      )}>
                        項目: {report.category}
                      </span>
                    )}
